...
### Cambios en las Excepciones

- **Refactorización de las Excepciones del Ajedrez**: Se ha refactorizado el sistema de excepciones para reducir la duplicación de código y mejorar la consistencia. Las excepciones ahora heredan de una única clase base `ChessException`, con mensajes predeterminados que se pueden personalizar si es necesario.

  - **Clase `ChessException`**: Clase base para todas las excepciones relacionadas con el ajedrez.
  - **`InvalidMoveException`**: Mensaje predeterminado cambiado a "Movimiento inválido".
  - **`OutOfBoundsException`**: Mensaje predeterminado cambiado a "La pieza está fuera del tablero".
  - **`PieceAlreadyCapturedException`**: Mensaje predeterminado cambiado a "La pieza ya ha sido capturada".
  - **`CheckException`**: Mensaje predeterminado cambiado a "El rey está en jaque".
  - **`CheckmateException`**: Mensaje predeterminado cambiado a "El rey está en jaque mate".
  - **`ColorException`**: Mensaje predeterminado cambiado a "Movimiento del color equivocado".
  - **`TurnException`**: Mensaje predeterminado cambiado a "Es el turno del otro jugador".
  - **`InvalidPositionException`**: Mensaje predeterminado cambiado a "La posición es inválida. Debe estar dentro del tablero".
  - **`InvalidPieceMovementException`**: Mensaje predeterminado cambiado a "Movimiento no válido para esta pieza".
  - **`CantEatKingException`**: Mensaje predeterminado cambiado a "No puedes capturar al Rey".
  - **`WrongTurnException`**: Mensaje predeterminado cambiado a "Es el turno del otro jugador".
  - **`GeneralChessError`**: Mensaje predeterminado cambiado a "Ha ocurrido un error en el juego de ajedrez".
  - **`PieceNotFoundError`**: Mensaje predeterminado cambiado a "Pieza no encontrada en el tablero".

### Cambios en las Pruebas de Excepciones

- **Refactorización de las Pruebas de Excepciones**: Se ha ajustado el archivo de pruebas para que sea compatible con el nuevo sistema de excepciones. Los mensajes esperados en las pruebas ahora coinciden con los nuevos mensajes predeterminados.

  - **Método `_test_exception`**: Ahora verifica el mensaje predeterminado de las excepciones, eliminando la necesidad de pasar mensajes personalizados en las pruebas.
  - **Pruebas Específicas**: Cada prueba ahora compara el mensaje de la excepción con el mensaje predeterminado actualizado para garantizar la consistencia con las nuevas definiciones de excepciones.
